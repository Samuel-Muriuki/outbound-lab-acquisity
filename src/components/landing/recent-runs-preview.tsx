              ...
              <HoverElectricBorder borderRadius={12}>
                <TiltedWrapper innerClassName="h-full">
                  <Link
                    ...
                    className="group flex h-full min-h-[148px] flex-col rounded-lg border border-border bg-surface-1 p-4 transition-colors duration-200 [transition-timing-function:var(--ease-out)] hover:border-transparent hover:bg-surface-2"
                  >
                ...
                {/* DeleteRunButton lives INSIDE HoverElectricBorder so
                  * the mouseleave doesn't fire when the cursor moves
                  * onto the trash icon (otherwise the electric border
                  * vanishes mid-card). z-20 keeps it above the canvas
                  * overlay (z-2). */}
                {isOwner && (
                  <DeleteRunButton
                    runId={run.id}
